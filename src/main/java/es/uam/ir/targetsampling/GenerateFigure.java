...
//            ML10M,
    ...
    private static final String[] Recommenders_Sorted = new String[]{
            "iMF (full)"
            , "iMF (test)"
            , "kNN (full/test)"
            , "kNN (full)"
        ...
        switch (figure) {
            case 1:
                generateFigure1(
                        N_FOLDS,
                        METRICS,
                        merge_dataset_sampler(DATASETS, SAMPLING)
                );
                break;
    ...
    public static void generateFigure1(
        ...
        for (String dataset : datasets) {
            String inFile = RESULTS_PATH + BIASED_PATH + dataset + "-" + TARGET_SAMPLING_FILE;
            String outFile = RESULTS_PATH + "figure1." + dataset + ".txt";
            PrintStream out = new PrintStream(outFile);

            final String trimmed_dataset = dataset.replace("-unpopular", "").replace("-popular", "");
            int fullTargetSize = TARGET_SIZES.get(trimmed_dataset);
            ...
            for (String metric : metrics) {
                Map<String, double[]> values = prepareMetricsForFigure1(inFile, metric, fullTargetSize);
    ...
    private static Map<String, double[]> prepareMetricsForFigure1(String inFile, String metric, int fullTargetSize)
            throws FileNotFoundException {
    ...
    public static void generateFigure2(
        ...
        for (String sample : SAMPLING) {
            String biasedFile = RESULTS_PATH + BIASED_PATH + YAHOO + "-" + sample + "-" + TARGET_SAMPLING_FILE;
            String unbiasedFile = RESULTS_PATH + UNBIASED_PATH + YAHOO + "-" + sample + "-" + TARGET_SAMPLING_FILE;
            String outFile = RESULTS_PATH + "figure2." + sample.replace("-", "") + ".txt";
            ...
            //Biased file
            Scanner in = new Scanner(new File(biasedFile.replace("--", "-")));
            String[] colHeads = in.nextLine().split("\t");
            ...
            //Unbiased file
            in = new Scanner(new File(unbiasedFile.replace("--", "-")));
    ...
    public static void generateFigure3(
        ...
        for (String dataset : datasets) {
            ...
            String outFile = RESULTS_PATH + outFileName + dataset + ".txt";
            PrintStream out = new PrintStream(outFile);
    ...
    private static Map<String, Map<Integer, Map<String, Double>>> prepare_for_figure3(
            String folder,
        ...
        for (String dataset : datasets) {
            ...
            Thread thread = new Thread(() -> {
                try {
                    ...
                    System.out.println("generateFigure4" + dataset + e);
                    e.printStackTrace(System.out);
                }
            });
            thread.start();
    ...
    private static void generateFigure4_sub(
            String biasedFolder,
            String unbiasedFolder,
            String[] metrics,
            String outFile,
    ...
    public static void generateFigure5(
        ...
        generateFigure3(folder, dataset_copy, metrics, nFolds, outFileName);
    }


    private static String[] getSortedMetrics(String[] recommenders) {
        Arrays.sort(recommenders, (r1, r2) -> {
            final int x = Arrays.asList(Recommenders_Sorted).indexOf(r1);
            final int y = Arrays.asList(Recommenders_Sorted).indexOf(r2);
            return Integer.compare(x, y);
        });
        return recommenders;
    }

    private static String[] merge_dataset_sampler(String[] datasets, String[] samplers) {

        final List<String> runs = new ArrayList();
        for (String dataset : datasets) {
            for (String sampler : samplers) {
                runs.add(dataset + (sampler != "" ? "-" + sampler : ""));
            }
        }

        return runs.toArray(new String[0]);
    }
    ...
    public static int getIndexOf(String[] items, String item) {
    ...
    private static String[] getSortedRecommenders(Map<String, double[]> values) {
        ...
        Arrays.sort(recommenders, Comparator.comparingInt(recommender -> getIndexOf(Recommenders_Sorted, recommender)));
        return recommenders;
    }
}
